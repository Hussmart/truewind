...
**This chart is also real data**: 400 days of real daily BTC/USDT closes from 4-5 major exchanges via `ccxt` (bybit, kucoin, mexc, bitget, poloniex — chosen because they don't block cloud/CI IP ranges; pass your own `exchanges=[...]` for others). Across ~400 daily observations the exchanges track within basis points of each other -- except **June 3, 2026**, in the middle of a sharp BTC sell-off, when **Poloniex quoted +1.05% above the 4-exchange consensus** for a single day. It's a real, unmodified data point, not an injected one -- but its *cause* is genuinely unclear from daily OHLCV alone: Poloniex's volume that day was actually in the top 5% of the whole 400-day window, so it wasn't thin liquidity. It could be a closing-price-calculation difference between exchanges, latency in cross-exchange arbitrage during a fast move, or a local order-flow imbalance; distinguishing those would need tick-level data this project doesn't have. Treat it as "the detector found a real, verified divergence and correctly flagged only that one day" -- not as proof of *why*. (The synthetic mode above still exists for tests/CI, where a fixed, reproducible ground truth is needed.)
